    ...
    <div className="min-h-screen bg-surface-app flex items-center justify-center p-4">
      <div className="bg-white p-10 rounded-lg border border-border-subtle max-w-md w-full text-center flex flex-col items-center gap-4">
        ...
        <div className="bg-surface-sidebar rounded-lg p-4 w-full text-left mt-2 flex flex-col gap-1">
          ...
          <p className="text-sm text-text-secondary">{selectedSlot && formatSlot(selectedSlot)} · {selectedType?.duration} min</p>
                ...
                <>
                  ...
                  {appointmentTypes.length > 1 && step === "calendar" && (
                    <button
                      ...
                      className="mt-4 text-xs font-semibold text-text-secondary hover:text-text-primary underline underline-offset-2 transition-colors"
                    >
            ...
            <div className="flex-1 p-8">
                ...
                <div className="flex flex-col gap-4">
                  <div className="mb-2">
                    {deal?.contact?.firstName ? (
                      <h2 className="text-2xl font-bold text-text-primary">¡Hola {deal.contact.firstName}! 👋</h2>
                    ) : (
                      <h2 className="text-2xl font-bold text-text-primary">¡Bienvenido! 👋</h2>
                    )}
                    <p className="text-text-secondary mt-1 text-sm">
                      {deal?.title ? (
                        <>Estás agendando una cita para: <strong className="text-text-primary">{deal.title}</strong>. Por favor selecciona el tipo de cita.</>
                      ) : (
                  ...
                  {appointmentTypes.length === 0 ? (
                    <p className="text-text-secondary text-sm">No hay tipos de cita disponibles.</p>
                  ) : (
                  ...
                  <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-6">
                        <h2 className="text-lg font-bold text-text-primary">{MONTHS[currentMonth.getMonth()]} {currentMonth.getFullYear()}</h2>
                        <div className="flex gap-1">
                          <button onClick={prevMonth} className="p-2 rounded-lg hover:bg-nav-hover text-text-secondary"><HugeiconsIcon icon={ArrowLeft01Icon} size={18} /></button>
                          <button onClick={nextMonth} className="p-2 rounded-lg hover:bg-nav-hover text-text-secondary"><HugeiconsIcon icon={ArrowRight01Icon} size={18} /></button>
                        </div>
                      </div>
                      <div className="grid grid-cols-7 gap-1 mb-2">
                        {DAYS.map(d => <div key={d} className="text-center text-xs font-semibold text-text-secondary py-1">{d}</div>)}
                      </div>
                    ...
                    {showTzPicker && (
                      <div className="absolute bottom-8 left-0 z-50 w-80 bg-white rounded-lg border border-border-subtle overflow-hidden">
                        ...
                        <div className="max-h-64 overflow-y-auto">
                          ...
                          {filteredTz.length === 0 && <p className="px-4 py-6 text-sm text-text-secondary text-center">Sin resultados</p>}
                        </div>
                      </div>
                    )}
                  </div>
              ...
              {step === "form" && (
                <div className="max-w-md">
                  <button onClick={() => setStep("calendar")} className="flex items-center gap-1 text-sm text-text-secondary hover:text-text-primary mb-6 transition-colors">
