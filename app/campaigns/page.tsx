...
import { Mail, Send, Clock, CheckCircle2, XCircle, RefreshCw, Trash2, Eye, Code2 } from "lucide-react";
                  ...
                  {displayed.map((camp) => (
                    <tr key={camp.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {camp.status === "DRAFT" && <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600"><Clock size={12} /> Draft</span>}
                        {camp.status === "SENDING" && <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700"><Send size={12} /> Sending...</span>}
                        {camp.status === "COMPLETED" && <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-green-50 text-green-700"><CheckCircle2 size={12} /> Completed</span>}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-3">
                          ...
                          <button onClick={() => setPreviewCampaign(camp)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Previsualizar correo"><Eye size={16} /></button>
                          <button onClick={() => handleDelete(camp)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar campaña"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-gray-100 flex items-start justify-between shrink-0">
              <div className="flex flex-col gap-1">
                <h3 className="text-base font-bold text-gray-900 flex items-center gap-2"><Eye size={16} className="text-gray-400" />Previsualización del correo</h3>
              ...
              <button onClick={() => setPreviewCampaign(null)} className="text-gray-400 hover:text-gray-600 p-1 shrink-0"><XCircle size={24} /></button>
            </div>
            <div className="flex-1 overflow-hidden p-4">
              <div className="w-full h-full rounded-xl border border-gray-200 overflow-hidden bg-white" style={{ minHeight: "420px" }}>
                {previewCampaign.body?.trim() ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full py-20 text-gray-400 gap-2"><Eye size={32} className="opacity-30" /><span className="text-sm">Esta campaña no tiene cuerpo HTML</span></div>
                )}
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between shrink-0">
              ...
              <button onClick={() => { setIsModalOpen(false); setIsPreviewMode(false); }} className="text-gray-400 hover:text-gray-600 p-1"><XCircle size={24} /></button>
            </div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-lg">
                      <button type="button" onClick={() => setIsPreviewMode(false)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${!isPreviewMode ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}><Code2 size={13} />Código</button>
                      <button type="button" onClick={() => setIsPreviewMode(true)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${isPreviewMode ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}><Eye size={13} />Previsualizar</button>
                    </div>
                  </div>
                  {!isPreviewMode ? (
                    ...
                    <div className="w-full rounded-xl border border-gray-200 overflow-hidden bg-white" style={{ minHeight: "260px" }}>
                      {htmlBody.trim() ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full py-16 text-gray-400 gap-2"><Eye size={28} className="opacity-40" /><span className="text-sm">Escribe HTML en el editor para previsualizar</span></div>
                      )}
                    </div>
                  )}
                </div>
