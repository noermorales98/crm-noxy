  ...
  return (
    <main className="flex-1 min-h-0 overflow-x-hidden overflow-y-auto px-4 py-5 sm:px-6 sm:py-6 bg-surface-app">
          ...
          <div className="bg-surface-elevated rounded-surface border border-border-subtle p-5 sm:p-8 max-w-4xl flex flex-col gap-8">
            ...
            {/* Google Calendar */}
            <div>
              <h2 className="text-lg font-bold text-text-primary mb-1">Google Calendar</h2>
              ...
              {googleMsg && (
                <div className={`mb-4 px-4 py-3 rounded-lg text-sm font-medium ${googleMsg.type === "success" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>
                  {googleMsg.text}
                </div>
              )}
              ...
              {googleStatus === null ? (
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-green-50 border border-green-100 text-sm text-green-700 font-medium">
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-lg border border-border-subtle text-sm text-text-secondary hover:bg-surface-sidebar transition-colors disabled:opacity-50"
                  >
                    Desconectar
                  </button>
                </div>
                ...
                <button
                  ...
                  disabled={googleLoading}
                  className="flex items-center gap-2 px-5 py-2.5 bg-white border border-border-subtle hover:bg-surface-sidebar text-text-primary text-sm font-medium rounded-lg transition-colors disabled:opacity-50"
                >
            ...
            {/* Timezone */}
            <div>
              <h2 className="text-lg font-bold text-text-primary mb-1">Zona Horaria</h2>
              <p className="text-sm text-text-secondary mb-5">
            ...
            {/* Email Notifications */}
            <div>
              <h2 className="text-lg font-bold text-text-primary mb-1">Notificaciones por Email</h2>
              <p className="text-sm text-text-secondary mb-5">
                Recibe un correo cada vez que alguien llene un formulario. Usa el SMTP configurado en la empresa vinculada al formulario.
              </p>
              <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-text-primary">Correo de notificación</label>
                <input
                  type="email"
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-lg border border-border-subtle bg-surface-sidebar focus:bg-white focus:outline-none focus:ring-1 focus:ring-border-subtle transition-all text-sm"
                />
                <p className="text-xs text-text-secondary">Include country code (e.g., +1, +34, +52).</p>
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-text-primary">CallMeBot API Key</label>
                <input
                  type="password"
