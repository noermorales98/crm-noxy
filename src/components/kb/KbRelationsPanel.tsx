      ...
      <div className="bg-white rounded-lg w-full max-w-lg mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border-subtle">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-text-primary">Agregar relación</h3>
          </div>
          <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-lg text-text-secondary hover:text-text-primary hover:bg-nav-hover transition-colors">
            <X size={15} />
          </button>
        </div>
        ...
        <div className="px-5 pt-4 flex flex-col gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-lg border border-border-subtle bg-surface-sidebar focus:bg-white focus:outline-none focus:border-border-subtle text-sm transition-all"
            />
          </div>
        </div>
            ...
            results.map((item) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-surface-sidebar transition-colors text-left w-full disabled:opacity-50"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-text-secondary">{item.typeLabel}</p>
                  </div>
                  {adding === item.id ? (
                    <div className="w-4 h-4 border-2 border-border-subtle border-t-gray-700 rounded-full animate-spin shrink-0" />
                  ) : (
...
export default function KbRelationsPanel({ pageId, initialRelations }: KbRelationsPanelProps) {
  ...
  if (collapsed) {
    return (
      <div className="w-8 bg-white border-l border-border-subtle flex flex-col items-center py-3 shrink-0">
        <button
        ...
        {relations.length > 0 && (
          <span className="mt-2 text-[10px] font-bold text-text-secondary">{relations.length}</span>
        )}
      </div>
    );
  }
  ...
  return (
    <>
      <div className="w-60 bg-white border-l border-border-subtle flex flex-col shrink-0 overflow-hidden">
        {/* Header */}
        <div className="px-4 py-3 border-b border-border-subtle flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-text-secondary uppercase tracking-wider">Relaciones</span>
            {relations.length > 0 && (
              <span className="text-[10px] font-bold bg-gray-100 text-text-secondary px-1.5 py-0.5 rounded-full">{relations.length}</span>
            )}
          </div>
          <div className="flex items-center gap-1">
            <button
              onClick={() => setShowModal(true)}
              className="w-6 h-6 flex items-center justify-center text-text-secondary hover:text-text-primary hover:bg-nav-hover rounded-lg transition-colors"
        ...
        <div className="flex-1 overflow-y-auto py-3 px-3">
          {Object.keys(grouped).length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 gap-2 text-center">
              <div className="w-10 h-10 rounded-lg bg-surface-sidebar flex items-center justify-center">
                <Link2 size={18} className="text-gray-200" />
              </div>
              <p className="text-xs text-text-secondary px-2">Conecta esta página con proyectos, formularios, clientes y más</p>
              <button
                onClick={() => setShowModal(true)}
                ...
                return (
                  <div key={type}>
                    <div className="flex items-center gap-1.5 mb-1.5 px-1">
                      ...
                      <span className="text-[10px] font-semibold text-text-secondary uppercase tracking-wider">{cfg.label}</span>
                    </div>
                    <div className="flex flex-col gap-0.5">
                      {grouped[type].map((rel) => (
                        <div key={rel.id} className="group flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-surface-sidebar transition-colors">
                          <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${cfg.color}`}>
                            <Icon size={11} />
                          </div>
                          <Link
                            href={cfg.href(rel.entityId)}
                            className="flex-1 min-w-0 text-xs text-text-primary font-medium truncate hover:text-blue-600 transition-colors"
                            title={displayRelationLabel(rel.entityType, rel.entityLabel)}
                          >
        ...
        {Object.keys(grouped).length > 0 && (
          <div className="px-3 py-2 border-t border-border-subtle shrink-0">
            <button
              onClick={() => setShowModal(true)}
              className="w-full flex items-center justify-center gap-1.5 py-1.5 text-xs text-text-secondary hover:text-text-primary hover:bg-surface-sidebar rounded-lg transition-colors"
            >
